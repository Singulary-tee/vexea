...
let dm = `import * as THREE from "three/webgpu";
...
export async function initDroneModels(scene: THREE.Scene): Promise<void> {
  ...
  fixedWingParts.forEach(p => {
      let obj = p.isMesh ? new THREE.Mesh(p.geom!, tMatAir) : new THREE.Group();
      obj.name = p.name;
...
ds = ds.replace(
  /const partsInfo = batch\.partsInfo;[\s\S]*?(?=}\n              }\n            })/m,
